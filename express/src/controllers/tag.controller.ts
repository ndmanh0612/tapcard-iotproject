...
export async function createEntity(req: express.Request, res: express.Response) {
  try {
    ...
    // Upsert tag: Nếu học sinh đã có thẻ thì cập nhật mã thẻ mới, nếu chưa thì tạo mới
    const existingTag = await TagModel.findOne({ associated });
    
    let result;
    if (existingTag) {
      // Cập nhật mã thẻ mới cho học sinh này
      existingTag.tag = tag;
      existingTag.by = req.body.auth.user._id;
      result = await existingTag.save();
    } else {
      // Tạo mới hoàn toàn
      result = await TagModel.create({ tag, associated, by: req.body.auth.user._id });
    }

    if (!result) return genericRes.badRequest(req, res, "Failed to bind tag.");
    return genericRes.successOk(req, res, result, "Tag bound successfully!");
  } catch (error) {
    return genericRes.badRequest(req, res, (error as Error).message, "Something went wrong!");
  }
}
...
export async function deleteEntity(req: express.Request, res: express.Response) {
  try {
    ...
    // Ensure requirements
    const tag = (req.query.tag || req.body.tag)?.toString().trim();
    if (!tag) return genericRes.badRequest(req, res, "Must provide tag ID.");
