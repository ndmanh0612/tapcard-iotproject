...
export async function toggleBindingMode(req: express.Request, res: express.Response) {
  try {
    if (req.body.auth.user.type != "admin") return genericRes.unauthorized(req, res, "Only admin can toggle binding mode.");
    
    const id = req.body.id?.toString().trim();
    const enable = req.body.enable === true;
    if (!id) return genericRes.badRequest(req, res, "Missing reader ID.");

    const reader = await ReaderModel.findByIdAndUpdate(id, { isBindingMode: enable }, { new: true });
    if (!reader) return genericRes.badRequest(req, res, "Reader not found.");

    return genericRes.successOk(req, res, reader, `Binding mode ${enable ? "enabled" : "disabled"} for ${reader.name}`);
  } catch (error) {
